...
from lumina_memory.kernel import Memory, superpose, reinforce, decay, forget, SALIENCE_REINFORCE_CAP
import numpy as np
...
def run_m3_validation():
    ...
    try:
        ...
        # Check associative properties - both should contain all original IDs
        left_lineage = set(left.lineage)
        right_lineage = set(right.lineage)
        expected_ids = {"a", "b", "c"}
        
        assert expected_ids.issubset(left_lineage), f"Left lineage missing IDs: {expected_ids - left_lineage}"
        assert expected_ids.issubset(right_lineage), f"Right lineage missing IDs: {expected_ids - right_lineage}"
