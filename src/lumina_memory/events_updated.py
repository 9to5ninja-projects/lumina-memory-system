...
@dataclass
class MigrationPayload(EventPayload):
    """Payload for MIGRATION events - embedding model migration."""
    old_model_version: str        # Previous model version
    new_model_version: str        # New model version
    migrated_count: int          # Number of memories migrated
    migration_strategy: str      # Migration strategy used
...
class IngestPayload(EventPayload):
    ...
    model_version: str            # Embedding model version (name + hash)
